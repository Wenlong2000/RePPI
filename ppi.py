...
def _ols_get_stats_X(
    ...
    if use_unlabeled:
        for i in range(N):
            grads_hat_unlabeled[i, :] = (
                w_unlabeled[i]
                * Xhat_unlabeled[i, :]
                * (np.dot(Xhat_unlabeled[i, :], pointest) - Y_unlabeled[i])
            )
    ...
    for i in range(n):
        hessian += (
            w[i] / n * np.outer(X[i], X[i])
        )
        grads[i, :] = w[i] * X[i, :] * (np.dot(X[i, :], pointest) - Y[i])
        grads_hat[i, :] = (
            w[i] * Xhat[i, :] * (np.dot(Xhat[i, :], pointest) - Y[i])
        )
...
def ppi_ols_pointestimate(
    ...
    if lhat is None:
        ...
        ppi_pointest = Sigma_inv @ (X.T @ Y / n
                                    + X_unlabeled.T @ Yhat_unlabeled / N  
                                    - X.T @ Yhat / n)
    else:
        Sigma = (1-lhat) * X.T @ X / n + lhat * X_unlabeled.T @ X_unlabeled / N
        Sigma_inv = np.linalg.inv(Sigma)
        ppi_pointest = Sigma_inv @ (X.T @ Y / n
                                    + lhat * X_unlabeled.T @ Yhat_unlabeled / N  
                                    - lhat * X.T @ Yhat / n)
